...
def calculate_new_probability(
    ...
    # 按比例调整其他选项的概率
    other_probabilities = {
        k: v for k, v in all_probabilities.items() if k != selected_outcome
    }
    ...
    if total_other > 0:
        for outcome in other_probabilities:
            ...
            all_probabilities[outcome] = max(
                0.01, min(0.99, all_probabilities[outcome])
            )
...
def display_question_info(question):
    ...
    st.markdown(
        f"**🕒 时间:** {question['created_at'].strftime('%Y-%m-%d %H:%M')} - {question['expire_at'].strftime('%Y-%m-%d %H:%M')}"
    )
    ...
    with col1:
        if st.button("👀 预估结果", use_container_width=True):
            if vote_type == "投票":
                ...
                st.info(
                    f"预计 {selected_outcome} 概率: {current_probability:.1%} → {new_probability:.1%}"
                )
            elif min_quantity > 0:
                changes = [
                    f"{outcome}: {question['probabilities'][outcome]:.1%} → {calculate_new_probability(question['probabilities'][outcome], amount, False, question['probabilities'].copy(), outcome):.1%}"
    ...
    with col2:
        if st.button("✅ 确认执行", type="primary", use_container_width=True):
            ...
            if vote_type == "投票":
                ...
                st.session_state.positions[question_id][selected_outcome] = (
                    st.session_state.positions[question_id].get(selected_outcome, 0)
                    + amount
                )
                ...
                if is_ok:
                    ...
                    for outcome in question["outcomes"]:
                        quantity = st.session_state.positions[question_id].get(
                            outcome, 0
                        )
